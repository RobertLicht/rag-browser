...
import { getState, setState, subscribe, addMessage, clearConversation } from './state.js';
import { loadEmbeddingModel, unloadEmbeddingModel, isEmbeddingLoaded } from './embedding.js';
import { loadLLM, unloadLLM, stopGeneration, isLLMLoaded } from './llm.js';
import { createDB, getDocumentCount, persistIndex, restoreIndex } from './orama-db.js';
...
export async function init() {
  ...
  if (restoredDb) {
    db = restoredDb;
    console.log(`Restored index from IndexedDB: ${getDocumentCount(db)} documents`);
  } else {
    db = createDB();
  }
...
async function handleFileUpload(files) {
  for (const file of files) {
    ...
    try {
      ...
      showNotification(`Failed to index ${file.name}: ${error.message}`, 'error');
    }
  }
}
...
async function handleQuery(query) {
  ...
  try {
    ...
    if (getDocumentCount(db) === 0) {
      ...
      emptyMsg.onToken('Please upload some .txt documents first so I have context to answer your questions.');
      emptyMsg.finalize();
      addMessage('assistant', 'Please upload some .txt documents first so I have context to answer your questions.');
      return;
    }
...
async function handleLoadModels() {
  ...
  if (!isLLMLoaded()) {
    setState({ models: { ...getState().models, llm: 'loading' } });
    showNotification('Loading LLM...', 'info');
